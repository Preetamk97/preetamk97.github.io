# Project 4: TurtleBot3 Navigation with Custom A* Planner & Diagnostic System
...
inside the directory where this file id currently located.
...
- The default A* Global Planner provided by the Navigation2 stack has a functionality of continously replanning the robot’s path as the robot keeps progressing towards its goal and its current position w.r.t the goal.
- However, in the Custom Global A* Planner, this functionality of continous replanning of robot’s path is not included. The path calculated by the planner only once at the very start when the robot is at the starting position/node and the calculated path remains fixed for the rest of the journey until the robot reaches its intended goal.
