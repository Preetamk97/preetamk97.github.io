# Chapter 25 — Creating a Service Server (C++)

[← Back to Contents](00_Contents.md) | [← Previous Lesson: Chapter 24 — Creating a Service Client (C++)](24_Creating_a_Service_Client_Cpp.md) | [Next Lesson: Chapter 26 — Creating Services (Python) →](26_Creating_Services_Python.md)
...
3. Add the following code to **service_server.cpp** file:
    ...
    ```cpp
    // Alternative format of code inspired from standard documentation

    #include "rclcpp/rclcpp.hpp"
    #include "cpp_srv/srv/odd_even_check.hpp"
    #include <memory>
    // 1. The Callback Function (Standard functional approach)void check_num_odd_even(  const std::shared_ptr<cpp_srv::srv::OddEvenCheck::Request> request,  std::shared_ptr<cpp_srv::srv::OddEvenCheck::Response> response){  // Calculate if the number is odd or even  // std::abs ensures negative numbers are handled correctly  int remainder = std::abs(request->number % 2);
      if (remainder == 0) {    response->decision = "Even";  } else {    response->decision = "Odd";  }
      // Log the activity to the console  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Incoming request: [%ld]", request->number);  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Sending back response: [%s]", response->decision.c_str());}
    int main(int argc, char **argv){  rclcpp::init(argc, argv);
      // 2. Create the Node  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("odd_even_check_server");
      // 3. Create the Service  // We pass the service name "odd_even_check" and the function pointer &check_num_odd_even  rclcpp::Service<cpp_srv::srv::OddEvenCheck>::SharedPtr service =    node->create_service<cpp_srv::srv::OddEvenCheck>("odd_even_check", &check_num_odd_even);
      RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Ready to check Odd/Even numbers.");
      // 4. Spin the node so it can listen for requests  rclcpp::spin(node);
      rclcpp::shutdown();  return 0;}
    ```

    > **💡 Note:** To remove the red squiggle line below the 2nd include statement: Click on the line → Click on the bulb icon appearing at the top → Under the **Quick Fix** select the 1st option **“Add to “includePath”:$…”**
    >
    > ![Figure 1 — Creating a Service Server (C++)](images/image139.png)
...
7. To run the **service server** node, open a new terminal in the **workspace** directory and run the following commands:
    ...
    ![Figure 2 — Creating a Service Server (C++)](images/image140.png)
...
8. Open a **parallel** terminal and run the following command:
    ...
    ![Figure 3 — Creating a Service Server (C++)](images/image141.png)
...
1. Ensure that the **service_server** node is already running on a different terminal. If not run the following commands from a **new terminal** in the **workspace** folder:
    ...
    ![Figure 4 — Creating a Service Server (C++)](images/image142.png)
...
2. Open a **2nd terminal** in the **workspace** folder and run the following commands:
    ...
    ![Figure 5 — Creating a Service Server (C++)](images/image143.png)
...
2. Open a **2nd terminal** in the **workspace** folder and run the **service_client** node:
    ...
    ![Figure 6 — Creating a Service Server (C++)](images/image144.png)
