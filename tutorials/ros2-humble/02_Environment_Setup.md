...
1. **(This step is not needed anymore) Troubleshooting For Future: (**[https://www.debugpoint.com/failed-connect-raw-githubusercontent-com-port-443/](https://www.debugpoint.com/failed-connect-raw-githubusercontent-com-port-443/)**)**
...
1. Click on **Manage Gear Icon** at the left bottom corner ****of VS Code Window **→ Settings →** Extend the tab **Extensions →** Click on **ROS2** (Under Extensions) → Write **humble** in the box under **ROS2: Distro**
...
The advantage of using the **Terminator** terminal is that - it allows us to open multiple instances of terminals and to manage and view all the opened terminals from a single window screen. This feature comes in handy while developing ROS2 applications, since here we will be working with lots terminals opened simultaneously. To install the Terminator Terminal:
...
# Installing Some Additional ROS2 Packages:

```bash
sudo apt-get install ros-humble-joint-state-publisher.gui

# This ROS2 Humble package provides us with a graphical user interface (GUI) 
# through which we will be able to interact and control the joints of a 
# robotic arm manipulator using ROS2.
```

```bash
sudo apt-get install ros-humble-xacro

# Installing ROS2 Humble Xacro package 
```

```bash
sudo apt-get install ros-humble-ros-gz*

# Installing ROS2 Humble Gazebo Packages that 
# provides us with the Gazebo Simulator
```

```bash
sudo apt-get install ros-humble-ros2-control

# Installing ROS2 Control package
```

```bash
sudo apt-get install ros-humble-ros2-controllers

# Installing ROS2 Controllers package
```
...
# Installing Some More Support Packages

```bash
sudo apt-get install libserial-dev

# This package allows us to write ROS2 C++ nodes
# for sending messages to the Arduino Board 
...
# This package (pyserial) will allow us to write python nodes 
# for sending messages to the Arduino Board
# It is equivalent of libserial-dev package that allows us to write ROS2 C++ node
# for sending messages to the Arduino Board 
# through the serial communication port.
```
...
# This python package (flask) will allow us to create
# a webserver on our PC.
```

```bash
pip install flask-ask-sdk

# This python package (flask-ask-sdk) will allow us to interface our webserver 
# (that we will create using the flask library) and hence our robot with Alexa.
```
