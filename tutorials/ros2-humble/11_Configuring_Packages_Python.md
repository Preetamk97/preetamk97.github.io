# Chapter 11 — Configuring Packages (Python)

[← Back to Contents](00_Contents.md) | [← Previous Lesson: Chapter 10 — Creating ROS2 Subscriber (Python)](10_Creating_ROS2_Subscriber_Python.md) | [Next Lesson: Chapter 12 — Configuring VS Code for Easy ROS2 Workspace Compilation (Python) →](12_Configuring_VSCode_for_ROS2_Workspace_Compilation_Python.md)

---

In the previous lessons, we have created our own ROS2 Python **Publisher** node (*publisher.py*) and **Subscriber** node (*subscriber.py*) which were able to communicate with each other using ROS2 **DDS.**
...
In this lesson, we will be looking at how we can configure our ROS2 package (*udemy_ros2_pkg*) so that we can **run our nodes** (*publisher.py & subscriber.py*) without having to navigate to the package’s **ros2_py_udemy_tutorial/src/udemy_ros2_pkg/scripts** directory every time - from any terminal opened from **ros2_py_udemy_tutorial** workspace folder - using **`ros2 run`** terminal command.
...
- The folder structure of **my_py_pkg** package (*which is an `ament_python` build-type package*) looks like below:
    ...
    ![Figure 1 — Configuring Packages (Python)](images/image71.png)
...
- We will store all our **.py scripts** for the ***my_py_pkg* package** inside the **my_py_pkg folder** of this package (*same names*). You will also notice that the **my_py_pkg** folder (that is inside the **my_py_pkg** package) already contains an **`__init__.py`** file inside it.
    ...
    ![Figure 2 — Configuring Packages (Python)](images/image72.png)
...
- Copy and paste the files **publisher.py** & **subscriber.py** from the **udemy_ros2_pkg/scripts** folder into the **my_py_pkg/my_py_pkg** folder.
    ...
    ![Figure 3 — Configuring Packages (Python)](images/image73.png)
...
We will not build the entire workspace of **ros2_py_udemy_tutorial** again. Instead, we will build only the **my_py_pkg**, selectively from the workspace directory.
