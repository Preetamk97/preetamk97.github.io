# Chapter 8 — Debugging and Compiling in VS Code (C++)

[← Back to Contents](00_Contents.md) | [← Previous Lesson: Chapter 7 — Configuring Packages (C++)](07_Configuring_Packages_Cpp.md) | [Next Lesson: Chapter 9 — Creating ROS2 Subscriber (C++) →](09_Creating_ROS2_Subscriber_Cpp.md)
...
- In this step, we are going to create **the bug**. Inside the **HelloWorldPubNode()** constructor of **publisher.cpp** file — comment out the **publisher** object initiation code line i.e.
    ...
    **publisher.cpp** code after this last step:
    ...
    > **💡 Note:** One thing to notice here is that, even though we have created an **error** in our code here, our VS Code Editor does not show us any indication of the error.
...
> **💡 Note:** Right now, our terminal is not aware of the **udemy_ros2_pkg** package that is inside the **src** folder of our **debug_ws** workspace - which can be checked using `ros2 pkg list` command. So to do this, we have to **source** the **install/setup.bash** of our **debug_ws** workspace. Next step is precisely for this purpose.
...
- Now run `ros2 run udemy_ros2_pkg publisher` command.
    ...
    ![Figure 1 — Debugging and Compiling in VS Code (C++)](images/image34.png)


Now, we don't see any error in our VS Code Editor. We don't see any error while compiling/building our workspace. Yet when we try to run our **publisher** executable using the `ros2 run <package_name> <executable_name>` command - it gives us this **error message** - **Segmentation fault** - that is hardly any helpful at all - in finding out what's wrong with our code and the location of the error.

So let's learn how we can better debug our ROS 2 code.
...
- After the workspace is successfully built, run the following code:
    ...
    > **💡 Note:** Here, we are using a popular debugger called **GNU Debugger (GDB)** while running our executable node **publisher.**
...
> **💡 Note:** You will get an error in the above command (as shown below) if you do not have **GDB server** installed on your Ubuntu system.
>
> ![Figure 2 — Debugging and Compiling in VS Code (C++)](images/image35.png)
>
...
![Figure 3 — Debugging and Compiling in VS Code (C++)](images/image36.png)

- Now we need to **configure** our **VS Code** to **communicate** with the **GDB server** that is listening to our **local machine (localhost)** on **port 3000.** So head back to the **VS Code Editor,** and under the **debug_ws/.vscode** folder — make a new file **launch.json.**
...
- **Save your work before proceeding** and now head back to **publisher.cpp** file in **VS Code Editor.**
    ...
    ![Figure 4 — Debugging and Compiling in VS Code (C++)](images/image37.png)
...
- **To close the debugger**, click the **Stop(Shift+F5)** button at left most of **debugger button panel.**
    ...
    ![Figure 5 — Debugging and Compiling in VS Code (C++)](images/image38.png)
...
- Rebuild the workspace using **colcon command:**
    ...
    ![Figure 6 — Debugging and Compiling in VS Code (C++)](images/image39.png)
...
- **Run** → **Start Debugging.**
    ...
    ![Figure 7 — Debugging and Compiling in VS Code (C++)](images/image40.png)
...
- Open a **new terminal in VS Code** and run the following command:
    ...
    > **💡 Note:** On running this command, sometimes you may run into an error that looks something like this:
    >
    > ![Error shown by `ros2 node list`](images/image41.png)
    >
    > *Picture source: [answers.ros.org — ros2 node list crash (XmlRpcClient fault)](https://answers.ros.org/question/407132/ros2-node-list-crash-xmlrpcclientfault/)*
    >
...
- The **build** directory is used as a temporary location for these **intermediate files**, and it is usually not necessary to keep them after the building process is complete.
...
![Figure 9 — Debugging and Compiling in VS Code (C++)](images/image42.png)

![Figure 10 — Debugging and Compiling in VS Code (C++)](images/image43.png)
...
![Figure 11 — Debugging and Compiling in VS Code (C++)](images/image44.png)
...
![Figure 12 — Debugging and Compiling in VS Code (C++)](images/image45.png)

![Figure 13 — Debugging and Compiling in VS Code (C++)](images/image46.png)

- Now every time we run `colcon build` command to build our workspace, the **ROS2 build system** looks for any changes in these **important changeable files** like **publisher.cpp** and **package.xml**, and copies the updated files over to the **install** directory.
...
- Now to make sure our `colcon build --symlink-install` command gets executed properly — first we need to **delete** our current **build, install** and **log** directories from our **debug_ws** workspace folder.

    ![Figure 14 — Debugging and Compiling in VS Code (C++)](images/image47.png)
...
- Now if we go to **debug_ws/install/udemy_ros2_pkg/share/udemy_ros2_pkg** we can see the **package.xml** file with an **arrow symbol** which shows that this file is actually a **symbolic link** to the **debug_ws/src/udemy_ros2_pkg/package.xml** file and not an exact copy of it.
    ...
    Any changes we make in the **debug_ws/src/udemy_ros2_pkg/package.xml** file will directly get reflected in **debug_ws/install/udemy_ros2_pkg/share/udemy_ros2_pkg/package.xml** file — without having to build the entire workspace.


![Figure 15 — Debugging and Compiling in VS Code (C++)](images/image48.png)

![Figure 16 — Debugging and Compiling in VS Code (C++)](images/image49.png)
...
- We can also see the same for **debug_ws/install/udemy_ros2_pkg/lib/udemy_ros2_pkg/publisher** file and **debug_ws/build/udemy_ros2_pkg/publisher** file.
    ...
    ![Figure 17 — Debugging and Compiling in VS Code (C++)](images/image50.png)

    ![Figure 18 — Debugging and Compiling in VS Code (C++)](images/image51.png)
...
- **For checking the location of the main source file**, we can use the `readlink file_name` command **on terminal** — **from the location of the symbolic link file** — that should be somewhere in the **install** directory.
    ...
    ![Figure 19 — Debugging and Compiling in VS Code (C++)](images/image52.png)

    ![Figure 20 — Debugging and Compiling in VS Code (C++)](images/image53.png)
...
![Figure 21 — Debugging and Compiling in VS Code (C++)](images/image54.png)
...
- Now click on **Terminal → Run Build Task**
    ...
    ![Figure 22 — Debugging and Compiling in VS Code (C++)](images/image55.png)

    or,

    click on **Terminal → Run Task →** choose **build**

    ![Figure 23 — Debugging and Compiling in VS Code (C++)](images/image56.png)

    ![Figure 24 — Debugging and Compiling in VS Code (C++)](images/image57.png)
...
- Now click on **Terminal → Run Task →** choose **debug**
    ...
    ![Figure 25 — Debugging and Compiling in VS Code (C++)](images/image56.png)

    ![Figure 26 — Debugging and Compiling in VS Code (C++)](images/image58.png)
...
- Type **ros.**
    ...
    ![Figure 27 — Debugging and Compiling in VS Code (C++)](images/image59.png)
