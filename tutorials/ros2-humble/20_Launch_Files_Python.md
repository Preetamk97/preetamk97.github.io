# Chapter 20 — Launch Files (Python)

[← Back to Contents](00_Contents.md) | [← Previous Lesson: Chapter 19 — Launch Files (C++)](19_Launch_Files_Cpp.md) | [Next Lesson: Chapter 21 — Project 2 — Launch Files (C++) →](21_Project_Launch_Files_Cpp.md)
...
2. Create a **new folder** named **launch** inside your **udemy_ros2_pkg** package folder.
    ...
    ![Figure 1 — Launch Files (Python)](images/image126.png)
...
3. **Right-click** on **launch → New File** → Name it **rpm_pub_node.launch.py** — this is a **python file (.py)** and inside this file, we are gonna add some **python code.** *(The **.launch** part is added **by convention** as a tribute to the ROS1 launch files which ended with .launch. You can also name this file as **rpm_pub_node_launch.py** and that is perfectly acceptable.)*
    ...
    ![Figure 2 — Launch Files (Python)](images/image127.png)

4. Add the following code to the newly created **rpm_pub_node.launch.py** file:
...
8. To run the **launch file** from the **terminal →** Open a new terminal in the **workspace** folder and run the following **bash commands:**
    ...
    ![Figure 3 — Launch Files (Python)](images/image128.png)

    Observe the highlighted portion of the output below:

    ![Figure 4 — Launch Files (Python)](images/image129.png)

    The **[INFO] [rpm_publisher.py-1]** message states that **rpm_publisher.py** node has been started successfully.

    Next in the queue - **[INFO] [ros2-2]** states that the `ros2 topic list` command has been run successfully.

    Next in the queue, there are two **[ros2-2]** messages which give the output of the `ros2 topic list` command.
...
9. To check if our **rpm_publisher** node is running properly - run the following commands from a **new terminal:**
    ...
    ![Figure 5 — Launch Files (Python)](images/image130.png)

    ![Figure 6 — Launch Files (Python)](images/image131.png)
